...
def _infer_single_category_label(demcodes: List[str], dem_lookup: Dict[str, Dict[str, str]]) -> str | None:
    cats = []
    for code in demcodes:
        info = dem_lookup.get(str(code).strip(), {})
        c = (info.get("category_en") or "").strip()
        if c:
            cats.append(c)
        ...
        if st.button("Run analytical query", key="run_analytical_query_btn"):
            ...
            try:
                ...
                if result.params.demcodes:
                    cat_label = _infer_single_category_label(result.params.demcodes, dem_lookup) or "Category"
                    st.write(f"Demographic: {cat_label} (all subgroups)")
                else:
                ...
                # show subgroup labels used (labels only, no codes)
                if result.params.demcodes:
                    labels_used = []
